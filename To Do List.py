...
def edit_task():
    global listbox_tasks
    try:
        index = listbox_tasks.curserlection()[0]
        old_task = listbox_tasks.get(index)
        new_task = simpoledialog.askstring("Edit Task", "Edit the selected task:", initialvalue=old_task)
        if new_task:
            listbox_tasks.delete(index)
            listbox_tasks.insert(index, new_task)
        except IndexError:
            message.showwarning("Warning", "Please select a task to edit")
