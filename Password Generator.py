...
def generate_password():
    ...
    if password_length < 6:
        messagebox.shoewwarning("Warning", "Password length should be atleast 6 characters")
        return
    
    characters = string.ascii_letters + string.digits + string.punctuation
    generated_password = ''.join(random.choice(characters) for _ in range(password_length))
    entry_generated_password.delete(0, tk.END)
    entry_generated_passwowrd.insert(tk.END, generated_password)
...
def main():
    ...
    heading_label = tk.Label(root, text="Password Generator", font=("Comicsanms", 12, "bold"))
    heading_label.grid(row=0, column=0, columnspan=4, pady=15)
